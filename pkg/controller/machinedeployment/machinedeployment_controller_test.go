...
func TestReconcile(t *testing.T) {
	...
	if err := updateMachineDeployment(c, deployment, func(d *clusterv1beta1.MachineDeployment) { d.Spec.Replicas = int32Ptr(5) }); err != nil {
	...
	if err := updateMachineDeployment(c, deployment, func(d *clusterv1beta1.MachineDeployment) { d.Spec.Template.Labels["updated"] = "true" }); err != nil {
		t.Errorf("error scaling machinedeployment: %v", err)
	}
	if err := c.Update(context.TODO(), deployment); err != nil {
		t.Errorf("error updating instance: %v", err)
